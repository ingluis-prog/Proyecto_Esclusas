...
function obtenerRolActual() {
   const rol = sessionStorage.getItem('user-role-pedro-miguel') || null;
   return rol;
}
...
function establecerRol(rol) {
   if (['admin', 'guest'].includes(rol)) {
      sessionStorage.setItem('user-role-pedro-miguel', rol);
      cerrarLoginModal();
   }
}
   ...
   if (!enComp) {
      ...
      if (btnComplementos) {
         btnComplementos.addEventListener('click', function(e) {
            e.preventDefault();
            sessionStorage.removeItem('user-role-pedro-miguel');
            sessionStorage.removeItem('admin-ip-pedro-miguel');
            mostrarLoginModal();
         });
      }
      return;
...
function verificarCredenciales(event) {
   ...
   if (ADMINS_AUTORIZADOS.includes(numeroEmpleado) && password === 'admin') {
      ...
      sessionStorage.setItem('admin-ip-pedro-miguel', numeroEmpleado);
      establecerRol('admin');
   } else {
...
function mostrarSelectorRol() {
   ...
   const adminEmpleado = sessionStorage.getItem('admin-ip-pedro-miguel') || 'Desconocida';
   const selectorHTML = `
...
function cambiarRol(nuevoRol) {
   if (['admin', 'guest'].includes(nuevoRol)) {
      sessionStorage.setItem('user-role-pedro-miguel', nuevoRol);
      location.reload();
   }
}
