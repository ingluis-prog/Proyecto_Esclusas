...
function obtenerRolActual() {
   const rol = sessionStorage.getItem('user-role-miraflores') || null;
   return rol;
}
...
function establecerRol(rol) {
   if (['admin', 'guest'].includes(rol)) {
      sessionStorage.setItem('user-role-miraflores', rol);
      cerrarLoginModal();
   }
}
   ...
   if (!enComp) {
      ...
      if (btnComplementos) {
         btnComplementos.addEventListener('click', function(e) {
            e.preventDefault();
            sessionStorage.removeItem('user-role-miraflores');
            sessionStorage.removeItem('admin-ip-miraflores');
            mostrarLoginModal();
         });
      }
      return;
...
function verificarCredenciales(event) {
   ...
   if (ADMINS_AUTORIZADOS.includes(numeroEmpleado) && password === 'admin') {
      ...
      sessionStorage.setItem('admin-ip-miraflores', numeroEmpleado);
      establecerRol('admin');
   } else {
...
function mostrarSelectorRol() {
   ...
   const adminEmpleado = sessionStorage.getItem('admin-ip-miraflores') || 'Desconocida';
   const selectorHTML = `
...
function cambiarRol(nuevoRol) {
   if (['admin', 'guest'].includes(nuevoRol)) {
      sessionStorage.setItem('user-role-miraflores', nuevoRol);
      location.reload();
   }
}
